...
#define GPIO_PWM_B IMX_GPIO_NR(1, 9)
...
/*
 * Backlight off before OS handover to avoid LCD fading
 */
...
#ifdef CONFIG_SECONDARY_ENV_OFFSET
...
int mmc_get_env_addr(struct mmc *mmc, int copy, u32 *env_addr) {
	...
	if (reg & PERSIST_SECONDARY_BOOT) {
		printf("U-BOOT:Secondary bootloader in use\n");
		offset = CONFIG_SECONDARY_ENV_OFFSET;
	} else {
		printf("U-BOOT:Primary bootloader in use\n");
